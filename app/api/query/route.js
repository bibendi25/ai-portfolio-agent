...
function tokens(str) {
  return (String(str).toLowerCase().match(/[a-z0-9]{3,}/g) || []).filter(t => !STOP.has(t));
}
...
export async function POST(req) {
  try {
    ...
    if (mode === 'ask') {
      ...
      // Doc-level fallback (always show *something*)
      if (!hits.length) {
        const scoredDocs = corpus
          .filter(d => d.text)
          .map(d => {
            const lower = d.text.toLowerCase();
            const docScore = qTok.reduce((s,t)=> s + (lower.includes(t) ? 1 : 0), 0);
            return { score: docScore, doc: d };
          })
          .filter(x => x.score > 0)
        ...
        for (const { doc } of scoredDocs) {
          const paras = splitParas(doc.text);
          if (paras[0]) hits.push({
            score: 1,
            p: `${titleFor(doc.path, doc.text)} — ${paras[0]}`,
            source: doc.path
          });
        }
      }
    ...
    if (mode === 'case') {
      ...
      const context = findSection('the\\s+brief');
      const role    = findSection('my\\s+role');
      const results = findSection('major\\s+accomplishments|challenges?\\s*&?\\s*results?');
