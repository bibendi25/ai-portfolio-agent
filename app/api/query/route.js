...
export async function POST(req) {
  ...
  if (mode === 'list') {
    ...
    return NextResponse.json({ projects, others, ok: true });
  }
  ...
  if (mode === 'ask') {
    ...
    for (const doc of corpus) {
      ...
      for (const p of paras) {
        const score = terms.reduce((s,t)=> s + (p.toLowerCase().includes(t) ? 1 : 0), 0);
  ...
  if (mode === 'cover') {
    ...
    const bio114 = corpus.find(c=> c.path.endsWith('bio/bio_114.md'))?.text || corpus.find(c=> c.path.endsWith('bio_114.md'))?.text || '';
    const bio48 = corpus.find(c=> c.path.endsWith('bio/bio_48.md'))?.text || corpus.find(c=> c.path.endsWith('bio_48.md'))?.text || '';
