...
export default function Page(){
  ...
  useEffect(()=>{ const t = setInterval(()=> setTipIdx(i=> (i+1)%tips.length), 5000); return ()=>clearInterval(t); }, [tips.length]);
  ...
  return (
    <div className="container">
      <div className="hero">
        ...
        <p className="muted">{cfg?.footerNote || 'All answers cite only the files listed in Sources. No external data is used.'}</p>
      </div>
        ...
        <div className="col">
          <div className="card">
            ...
            <input
              ...
              onKeyDown={(e)=>{ if(e.key==='Enter'){ (async()=>{ setLoadingAsk(true); const resp = await api({mode:'ask', q}); setLoadingAsk(false); if(!resp.ok && resp.error){ alert(resp.error); return; } setHits(resp.hits || []); })(); }}}
            ...
            {hits?.length ? hits.map((h,i)=> (
              <div key={i} className="answer">
                {h.p}
                <div className="source">Source: <a href={`/docs/${h.source}`} target="_blank" rel="noreferrer"><code>{h.source}</code></a></div>
              </div>
          ...
          <div className="card">
            ...
            <div className="btnRow">
              <button className="btn primary" onClick={async()=>{
                ...
                if(!resp.ok && resp.error){ alert(resp.error); return; }
                setNote(resp.note || '');
              }}>{loadingCover ? <span className="spinner" /> : 'Generate'}</button>
